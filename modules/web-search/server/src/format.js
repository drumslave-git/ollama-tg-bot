export function formatWebSearchContext(query, payload) {
    const parts = [`Web search for "${query}":`];
    if (payload.answer) {
        parts.push(`\nSummary:\n${payload.answer}`);
    }
    ...
    if (!payload.answer && payload.results.length === 0) {
        return (`Web search was run for "${query}" but returned no results.\n` +
...
export function extractWebSearchSources(payload) {
    const sources = [];
    const seen = new Set();
...
export function formatWebSearchFailure(query, err) {
    const detail = err instanceof Error ? err.message : String(err);
    return (`Web search was attempted for "${query}" but failed: ${detail}\n\n` +
        `Tell the user live lookup failed. Do not pretend you searched successfully.`);
}
export function normalizeTavilyResults(rows) {
    const results = [];
