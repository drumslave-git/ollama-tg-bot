import { asObject, parseJsonContent, readStringArray, strictObjectSchema, } from "@llm-tg-bot/modules-utils";
export const MEMORY_EXTRACT_RESPONSE_FORMAT = strictObjectSchema("memory_extract", {
    user_facts: {
        type: "array",
        items: { type: "string" },
        description: "New durable facts about the current speaker only; empty array when nothing new.",
    },
    group_facts: {
        type: "array",
        items: { type: "string" },
        description: "New durable facts about the group/chat itself; empty array when nothing new.",
    },
    general_facts: {
        type: "array",
        items: { type: "string" },
        description: "New cross-chat general knowledge; empty array when nothing new.",
    },
...
function formatStored(kind, facts) {
    const content = facts.join("\n").trim();
    if (!content)
        return `(none yet for this ${kind})`;
    return content;
}
function normalizeFactLines(items) {
    if (!items)
        return [];
    return items
...
export function parseMemoryExtract(raw) {
...
export function buildMemoryExtractMessages(input) {
    ...
    return [
        ...
        {
            role: "user",
            content: `Already stored about this user:\n${userBlock}\n\n` +
