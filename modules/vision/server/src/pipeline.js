import { isBase64MediaHistoryContent, mediaKindForMessage, replaceBase64WithVisionDescription, } from "@llm-tg-bot/modules-history";
function buildVisionTurnBody(messageText, mediaKind, visionDescription) {
    const mediaNote = `The user sent a ${mediaKind}: ${visionDescription}`;
    return [messageText, mediaNote].filter(Boolean).join("\n\n");
}
export const visionReplyHost = {
    id: "vision",
    stepId: "vision",
    phase: "pre-reply",
    order: 5,
    ...
    shouldRun(state, services) {
        if (!state.shouldReply)
            return false;
        const msg = state.telegram.message;
        if (services.callbacks.messageHasVisionMedia?.(msg))
            return true;
        if (services.callbacks.findReplyMediaMessage?.(msg))
            return true;
        return false;
    },
    async run(state, services) {
        ...
        if (loaded.images.length === 0) {
            const replyMediaMsg = cb.findReplyMediaMessage?.(msg);
            if (replyMediaMsg) {
                const replyLoaded = await cb.loadVisionFromMessage?.(botToken, replyMediaMsg);
        ...
        const visionDescription = (await cb.describeVisionImages?.(loaded.images, { ...msgLog, fromReply: visionFromReply }, loaded.visionHint, state.turnId)) ?? "";
        ...
        const mediaKind = mediaKindForMessage(msg, !!sticker || !!loaded.sourceSticker);
        if (visionDescription && mediaOnCurrentMessage) {
            state.latestBody = buildVisionTurnBody(messageText, mediaKind, visionDescription);
        ...
        if (state.convKey && mediaOnCurrentMessage) {
            cb.mapHistoryBase64Media?.(state.convKey, isBase64MediaHistoryContent, (content) => replaceBase64WithVisionDescription(content, visionDescription));
        }
