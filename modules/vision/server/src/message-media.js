...
import { loadStickerForVision, stickerUnavailableText, } from "./stickers.js";
/** Download photo, image document, or sticker from a Telegram message for LLM vision. */
export async function loadVisionFromMessage(token, message) {
...
export function findReplyMediaMessage(message, maxDepth = 4) {
    let current = message.reply_to_message;
    let depth = 0;
    while (current && depth < maxDepth) {
        if (messageHasVisionMedia(current))
            return current;
...
export function messageHasVisionMedia(message) {
    if (message.photo?.length)
        return true;
    if (message.sticker)
        return true;
...
export function messageHasUserImage(message) {
    if (message.photo?.length)
        return true;
    if (message.document?.mime_type?.startsWith("image/"))
        return true;
