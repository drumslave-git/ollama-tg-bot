import { asObject, parseJsonContent, readInt, strictObjectSchema, } from "@llm-tg-bot/modules-utils";
import { MOOD_KEYS, MOOD_TRAIT_HINTS, normalizeMoodValues, } from "./values.js";
const moodProperties = Object.fromEntries(MOOD_KEYS.map((key) => [
    key,
    {
        type: "integer",
        minimum: 0,
...
export const MOOD_RESPONSE_FORMAT = strictObjectSchema("mood_traits", moodProperties, [...MOOD_KEYS]);
...
function formatCurrentMood(mood) {
    return MOOD_KEYS.map((key) => `${key}: ${mood[key]}`).join("\n");
}
function parseTraitsFromObject(obj, fallback) {
    const partial = {};
...
export function parseMoodBlock(raw, fallback) {
...
export function buildMoodEvaluateMessages(input) {
    const fallback = normalizeMoodValues(input.currentMood);
    const traitGuide = MOOD_KEYS.map((key) => `- ${key}: ${MOOD_TRAIT_HINTS[key]}`).join("\n");
    const userContent = `Current mood (starting point):\n${formatCurrentMood(fallback)}\n\n` +
        `Trait guide:\n${traitGuide}\n\n` +
