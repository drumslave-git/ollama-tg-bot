function normalizeEmojiMatch(value) {
    return value.normalize("NFC").trim().replace(/\uFE0F/g, "");
}
function emojisMatch(a, b) {
...
export function resolveStickerFileId(raw, stickers) {
    ...
    if (byEmoji.length === 1)
        return byEmoji[0].fileId;
    if (byEmoji.length > 1)
        return pickRandom(byEmoji)?.fileId ?? null;
    return null;
}
