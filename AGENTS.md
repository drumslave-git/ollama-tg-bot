...
- **Runner** (`server/src/runtime/browser-agent-runner.ts`, started from `server/src/index.ts`, paused in maintenance mode): a concurrency-limited (`browserAgentConcurrency`) worker. Each run gets its **own** `BrowserSession` (`session.ts` — one Playwright context/page from the shared Chromium singleton in `features/link-fetch/playwright.ts`; **not** a global — runs may overlap), runs an autonomous agent loop (`agent.ts` → `chatCompleteWithTools` with the browser tools, bounded by `browserAgentMaxSteps` rounds and a `browserAgentMaxSeconds` wall-clock timeout), then delivers the report out-of-band via `getBot()` + `appendAssistantMessage` (mirrors `features/tasks/fire.ts`). The session is closed in `finally`.
