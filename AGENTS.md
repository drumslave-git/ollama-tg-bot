...
- Debug (message processings): `server/src/debug/message-report.ts`, `server/src/db/debug/message-processing.ts` — `message_processings` (one-to-one with `chat_messages` by `chat_message_id`, FK `ON DELETE CASCADE`) + `message_processing_entries` (append-only `{title, type: 'text'|'json', content}`, FK `ON DELETE CASCADE`); 50 processings per chat (trimmed). Shared primitive `reportMessageProcessing(id, title, type, content)` where `id` = `chat_messages.id`; the live `MessageProcessingReport` (keyed by ephemeral `turnId`, linked to the stored chat message via `linkProcessingMessage`) buffers entries until the message is stored, then streams them. Each LLM call splits into **LLM request** (json body) → **Waiting for LLM** → **LLM response** (json body) + **LLM result** entries; terminal status is `processing`→`processed`/`ignored`/`error` with `total_time_spent`. Dashboard nav: chats → processings → entries (entries ordered by `created_at`, rendered by `type`). `/explain` resolves a processing by the bot's reply message id (`reply_message_ids`). Dashboard sidebar shows queue size + memory/vision job status (scheduled jobs show live countdown via `memoryJobRunAt` / `visionJobRunAt`) via `dashboard:stats`
