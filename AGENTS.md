...
- **Current time is given in two forms** in the `[SESSION]` block: `utc now:` (ISO-8601 UTC — for history tools' `from`/`to` date ranges) and `current time:` (the local wall clock in `config.timezone`, named). Scheduled-task times (`tasks_*` `time`, HH:MM) are interpreted in the bot timezone, so the model **must** compute relative times ("in 5 min", "at 18:00") from the local `current time:`, not the UTC value — feeding only UTC made it miscompute and create junk past-dated tasks. `extractSessionBlock` (reused by the tool loop) terminates on the `current time:` line, so that line stays **last** and the UTC line sits above it inside the captured block.
- The latest-turn user message is assembled as **strict, ordered blocks** in `buildLatestTurnMessage` (`server/src/pipeline/chat-messages.ts`): `[RECENT CHAT]` (background window, excludes the current message) → `[CURRENT SPEAKER]` → `[MENTIONED USERS]` → `[REPLY CONTEXT]`/`[REPLY THREAD]` → `[CURRENT MESSAGE]`. `[CURRENT MESSAGE]` always carries the literal body as the single thing to answer — without it the model answers the tail of the window when a reply jumps back to an earlier topic. Both the main reply and the tool-selection pass share this same user content, so the structure fixes both. Keep these labels in sync with the guidance in `BASE_SYSTEM_PROMPT_CORE` and `buildToolRoundSystemPrompt` if you rename a block.
