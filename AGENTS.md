...
- Chat options: `server/src/settings/limits.ts` (`temperature`, `topP`, `numCtx` via `getProviderExtensions()`). Only standard OpenAI-compatible fields are sent — Ollama-native `top_k`/`repeat_penalty`/`chat_template_kwargs` were dropped so any OpenAI-compatible backend works; generation cap is `max_tokens` (not `max_completion_tokens`, which Ollama's compat layer ignores).
