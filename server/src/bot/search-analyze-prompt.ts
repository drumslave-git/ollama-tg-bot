...
/**
 * Pure prompt + parsing helpers for the web-search decision analyzer.
 * No DB/LLM imports so they can be unit-tested and driven by live tests.
 */
export const SEARCH_ANALYZER_SYSTEM = `You decide whether a Telegram bot should run a web search (Tavily) before answering.
...
export function parseSearchDecision(raw: string): SearchDecision {
...
/** Build the web-search decision prompt (system + user) for the analyzer LLM call. */
export function buildSearchAnalyzerMessages(
